...
# API Endpoints
Except for [Involved People List](#involved-people-list) endpoint, all endpoints operations have `$isSuccess` boolean to indicate success of the operation. If it is `true` then it is successful, `false` if not and it means there was an error.
